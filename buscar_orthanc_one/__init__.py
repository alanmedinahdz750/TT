...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        try:
            query = "SELECT instancia, BodyPartExamined, Modality FROM estudios_orthanc WHERE Modality LIKE %s OR StudyDescription LIKE %s OR BodyPartExamined LIKE %s OR SeriesDescription LIKE %s OR informacion LIKE %s ORDER BY RAND() LIMIT 5"
